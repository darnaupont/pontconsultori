...
//jquery
(0, _jquery2.default)('.lang .' + lang).addClass("active");
...
function createslider(postsData, pItemID) {
  ...
  for (var i = 0; i < postsData.length; i++) {
    ...
    ourHTMLString += '<li class="' + active + ' orbit-slide" data-slide="0"' + style + '>' + '<figure class="orbit-figure">' + '<img class="orbit-image" src="' + postsData[i].media_details.sizes.large.source_url + '" alt="' + postsData[i].title.rendered + '">' + '<p class="text-right">' + languageText(lang, postsData[i].caption.rendered) + '</p>' + '</figure>' + '</li>';
  };
  pItemID.innerHTML = ourHTMLString;
  _foundationSites2.default.reInit('orbit');
};

function languageText(lang, text) {
  var esX = text.search("<:es>");
  var caX = text.search("<:ca>");
  var EndTagX = void 0;
